...
var DOM_ELEMENTS = {
  ...
  SLIDER_SETTINGS = {
    ...
    gap: Number(
      getComputedStyle(DOM_ELEMENTS.sliderInner).columnGap.replace("px", "")
    ),
    ...
    transformValueMax: function () {
      return (
        this.transformValue() *
        (this.mediaQuery ? this.countCards - 1 : this.countCards - 3)
      );
    },
  },
  SLIDER_VALUE = { acc: 0, elapsedMs: 0, intervalId: null },
  OBSERVER_SETTINGS = { root: null, rootMargin: "0px", threshold: 0.001 },
  OBSERVER_ELEMENTS = [
    DOM_ELEMENTS.statue,
  ...
  SCROLL = {
    ...
    scrollOn: function () {
      (document.body.style.cssText = ""),
        window.scrollTo({ top: scroll.position }),
        (document.documentElement.style.scrollBehavior = "");
    },
  },
  observer = new IntersectionObserver(observerCallback, OBSERVER_SETTINGS);
function burger() {
  var e = DOM_ELEMENTS.menu.classList.contains("menu-active");
...
function slide() {
  (SLIDER_VALUE.elapsedMs += 10),
    SLIDER_VALUE.elapsedMs >= SLIDER_SETTINGS.slideTime &&
      ((SLIDER_VALUE.acc += SLIDER_SETTINGS.transformValue()),
      SLIDER_VALUE.acc > SLIDER_SETTINGS.transformValueMax() &&
        ((SLIDER_VALUE.acc = 0),
        alert(
          "acc("
            .concat(SLIDER_VALUE.acc, ") > max value(")
            .concat(SLIDER_SETTINGS.transformValueMax(), ")")
        )),
      (DOM_ELEMENTS.sliderInner.style.transform = "translateX(-".concat(
        SLIDER_VALUE.acc,
        "px)"
      )),
...
function start() {
  alert("start");
  pause(), (SLIDER_VALUE.intervalId = setInterval(slide, SLIDER_SETTINGS.step));
}
function pause() {
  alert("pause");
...
function restart() {
  alert("restart")((SLIDER_VALUE.acc = 0)),
    (SLIDER_VALUE.elapsedMs = 0),
    (DOM_ELEMENTS.sliderInner.style.transform = "translateX(-0px)"),
    start();
}
function observerCallback(e, t) {
  var n = {
...
function addObserverElement(e) {
  e.forEach(function (e) {
    e instanceof NodeList
      ? e.forEach(function (e) {
          return observer.observe(e);
        })
      : e instanceof Element && observer.observe(e);
  });
}
function updateSettingsSlider() {
  alert("update")(
    ...
    (SLIDER_SETTINGS.mediaQuery = window.matchMedia(
      "(max-width: 1050px)"
    ).matches);
}
DOM_ELEMENTS.menuElements.forEach(function (e) {
  return e.addEventListener("click", burger);
}),
  ...
  window.addEventListener("resize", function () {
    alert("resize"), restart(), updateSettingsSlider();
  }),
  start(),
